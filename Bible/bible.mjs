...
for (let i = 0; i < books1.length; i++) {
  let book = {};
  // book.id = books1[i].id;
  book[books1[i].name] = books2[i].passage;
  book.chapters = [];

  for (let j = 0; j < books2[i].chapters.length; j++) {
    let obj = {};
    obj.chapter = j + 1;
    // obj.current = `${books1[i].id}.${j + 1}`;
    // obj.next = books2[i].chapters[j + 1] === undefined ? `${books1[i + 1]?.id}.1` : `${books1[i].id}.${j + 2}`;
